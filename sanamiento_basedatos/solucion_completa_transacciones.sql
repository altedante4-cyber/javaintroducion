...
-- ============================================
-- Script con transacciones (COMMIT/ROLLBACK)
-- Ejecutar después de 00_setup_gha.sql
...
-- Formato: "NIF</minimax:tool_call></minimax:tool_call>_ASSERTFechaVisita|Coste"
START TRANSACTION;
...
-- ============================================
-- PROCEDURE CON MANEJO DE ERRORES COMPLETO
-- ============================================
DELIMITER //

DROP PROCEDURE IF EXISTS sanitize_gha_analytics//

CREATE PROCEDURE sanitize_gha_analytics()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SELECT 'ERROR: Transacción revertida automáticamente' AS status;
        GET DIAGNOSTICS CONDITION 1 @errno = MYSQL_ERRNO, @errmsg = MESSAGE_TEXT;
        SELECT CONCAT('Error Code: ', @errno, ' - ', @errmsg) AS error_detail;
    END;

    SELECT '========================================' AS '';
    SELECT 'INICIO DEL SANEAMIENTO' AS '';
    SELECT '========================================' AS '';
    SELECT NOW() AS fecha_inicio;

    SELECT '1. Normalizando identidad de pacientes...' AS '';
    
    UPDATE pacientes SET nif = TRIM(nif);
    UPDATE pacientes SET nif = REPLACE(nif, '-', '');
    
    DELETE FROM pacientes WHERE nif IS NULL OR nif = '' OR nif = 'NULL_NIF' OR TRIM(nif) = '';
    
    DELETE p1 FROM pacientes p1 INNER JOIN pacientes p2 
    WHERE p1.nif = p2.nif AND p1.id > p2.id;
    
    DELETE FROM pacientes WHERE nif NOT REGEXP '^[0-9]{8}[A-Z]$';
    
    SELECT '2. Normalizando colegiados de médicos...' AS '';
    ...
    UPDATE medicos SET num_colegiado = CONCAT('COL-', SUBSTRING_INDEX(num_colegiado, '-', 1), '-', LPAD(SUBSTRING_INDEX(num_colegiado, '-', -1), 4, '0')) WHERE num_colegiado REGEXP '^[0-9]+-[0-9]+$' AND num_colegiado NOT LIKE 'COL-%';
    
    UPDATE medicos SET num_colegiado = CONCAT('COL-', SUBSTRING(num_colegiado, 4, 2), '-', LPAD(SUBSTRING(num_colegiado, 6), 4, '0')) WHERE num_colegiado REGEXP '^COL[0-9]{6}$';
    
    UPDATE medicos SET num_colegiado = CONCAT('COL-00-', LPAD(SUBSTRING(num_colegiado, -3), 4, '0')) WHERE num_colegiado NOT LIKE 'COL-%-%';
    
    SELECT '3. Verificando integridad referencial...' AS '';
    ...
    SELECT '4. Dividiendo tablas (seguros_pacientes)...' AS '';
    ...
    INSERT INTO seguros_pacientes (paciente_id, num_poliza)
    SELECT id, num_poliza FROM pacientes WHERE num_poliza IS NOT NULL AND num_poliza != '';
    
    ALTER TABLE pacientes DROP COLUMN num_poliza;
    
    SELECT '5. Procesando importes y copagos...' AS '';
    ...
    UPDATE visitas SET importe_sucio = REPLACE(importe_sucio, 'EUR', '');
    UPDATE visitas SET importe_sucio = REPLACE(importe_sucio, ',', '.');
    UPDATE visitas SET importe_sucio = NULL WHERE importe_sucio = '' OR UPPER(importe_sucio) = 'GRATIS';
    ...
    SELECT '6. Saneamiento profundo (emails, fechas)...' AS '';
    
    UPDATE pacientes SET email = 'desconocido@sinemail.com' WHERE email IS NULL OR email = '';
    UPDATE pacientes SET f_nacimiento = TRIM(f_nacimiento);
    UPDATE pacientes SET f_nacimiento = REPLACE(f_nacimiento, '.', '/');
    UPDATE pacientes SET f_nacimiento = NULL WHERE f_nacimiento = '' OR f_nacimiento IS NULL;
    UPDATE visitas SET fecha_visita = TRIM(fecha_visita);
    UPDATE visitas SET fecha_visita = REPLACE(fecha_visita, '.', '-');
    ALTER TABLE visitas MODIFY descuento_aplicado DECIMAL(10,2) DEFAULT 0;
    UPDATE visitas SET descuento_aplicado = 0 WHERE descuento_aplicado IS NULL;
    
    SELECT '7. Creando índices de optimización...' AS '';
